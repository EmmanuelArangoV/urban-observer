...
export async function login(email, password) {
    ...
    try {
        ...
        if (users.length === 0 || !Array.isArray(users)) {
            console.warn('⚠️ [AUTH SERVICE] Usuario no encontrado o credenciales inválidas');
...
export async function register(userData) {
    ...
    try {
        ...
        if (Array.isArray(existingUsers) && existingUsers.length > 0) {
            console.warn('⚠️ [AUTH SERVICE] Email ya registrado');
...
export function getActiveUser() {
    ...
    if (user) {
        ...
        console.log('⚠️ [AUTH SERVICE] No hay usuario activo');
    }
...
export function logout() {
    ...
    console.log('🗑️ [AUTH SERVICE] Usuario removido de localStorage');
